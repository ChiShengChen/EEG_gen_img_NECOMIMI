...
# class NervFormerV2_unit(nn.Module):
#     def __init__(self, num_channels=63, sequence_length=250, num_subjects=1, feature_dim=1024):
#         super(NervFormerV2_unit, self).__init__()
#         self.attention_model = EEGAttention(num_channels, num_channels, nhead=1)
#         self.subject_wise_linear = nn.ModuleList([nn.Linear(sequence_length, sequence_length) for _ in range(num_subjects)])
#         self.enc_eeg = NervFormer_EEG(output_dim=feature_dim)
#         self.proj_eeg = Proj_eeg_Muse(embedding_dim=feature_dim)
#         self.decoder = nn.Sequential(
#             nn.Linear(feature_dim, num_channels * sequence_length),
#             nn.ReLU()
#         )
#         self.logit_scale = nn.Parameter(torch.ones([]) * np.log(1 / 0.07))
#         self.loss_func = ClipLoss()
...
#             xi = x[i]
#             xi = self.subject_wise_linear[0](xi)
#             outputs.append(xi)
...
#         x_reconstructed = x_reconstructed.view(batch_size, num_channels, sequence_length)
#         return out, x_reconstructed

class NervFormerV2_unit(nn.Module):
    def __init__(self, num_channels=63, sequence_length=250, num_subjects=1, feature_dim=1024, enc_eeg=None):
        super(NervFormerV2_unit, self).__init__()
        self.attention_model = EEGAttention(num_channels, num_channels, nhead=1)
        self.subject_wise_linear = nn.ModuleList([nn.Linear(sequence_length, sequence_length) for _ in range(num_subjects)])
        if enc_eeg is None:
            ...
            self.enc_eeg = enc_eeg  # 使用共享的 NervFormer_EEG 实例
        self.proj_eeg = Proj_eeg_Muse(embedding_dim=feature_dim)
        self.decoder = nn.Sequential(
            nn.Linear(feature_dim, num_channels * sequence_length),
            nn.ReLU()
...
class StackedNervFormerV2_2L(nn.Module):
    def __init__(self, num_layers=2, num_channels=63, sequence_length=250, num_subjects=1, **kwargs):
        super(StackedNervFormerV2_2L, self).__init__()
        self.num_layers = num_layers
        self.nerv_formers = nn.ModuleList([
            NervFormerV2_unit(num_channels=num_channels, sequence_length=sequence_length, num_subjects=num_subjects, **kwargs)
            for _ in range(num_layers)
        ])
        self.logit_scale = nn.Parameter(torch.ones([]) * np.log(1 / 0.07))
        self.loss_func = ClipLoss()
        self.final_proj = nn.Linear(1024, 1024)  # 假设每个 NervFormerV2 的输出维度是 1024

    def forward(self, x, subject_indices=0):
        # x 的初始形状为 [batch_size, num_channels, sequence_length]
        for i, model in enumerate(self.nerv_formers):
            out, x = model(x, subject_indices)
            # 如果不是最后一层，使用解码器的输出作为下一层的输入
            if i != self.num_layers - 1:
                x = x  # x 已经是 [batch_size, num_channels, sequence_length]
        # 最后一层的输出
        out = self.final_proj(out)
        return out

# class StackedNervFormerV2_4L(nn.Module):
#     def __init__(self, num_layers=4, num_channels=63, sequence_length=250, num_subjects=1, **kwargs):
#         super(StackedNervFormerV2_4L, self).__init__()
#         self.num_layers = num_layers
#         self.nerv_formers = nn.ModuleList([
#             NervFormerV2_unit(num_channels=num_channels, sequence_length=sequence_length, num_subjects=num_subjects, **kwargs)
#             for _ in range(num_layers)
...
class StackedNervFormerV2_4L(nn.Module):
    def __init__(self, num_layers=4, num_channels=63, sequence_length=250, num_subjects=1, feature_dim=1024, **kwargs):
        super(StackedNervFormerV2_4L, self).__init__()
        self.num_layers = num_layers
        # 创建共享的 NervFormer_EEG 实例
        shared_enc_eeg = NervFormer_EEG(output_dim=feature_dim)
        self.nerv_formers = nn.ModuleList([
            NervFormerV2_unit(
                ...
                enc_eeg=shared_enc_eeg,  # 传递共享的实例
                **kwargs
            )
            for _ in range(num_layers)
        ])
        self.logit_scale = nn.Parameter(torch.ones([]) * np.log(1 / 0.07))
        self.loss_func = ClipLoss()
        self.final_proj = nn.Linear(feature_dim, feature_dim)  # 假设每个 NervFormerV2 的输出维度是 feature_dim
...
class StackedNervFormerV2_4L_res(nn.Module):
    def __init__(self, num_layers=4, num_channels=63, sequence_length=250, num_subjects=1, feature_dim=1024, **kwargs):
        super(StackedNervFormerV2_4L_res, self).__init__()
        self.num_layers = num_layers
        shared_enc_eeg = NervFormer_EEG(output_dim=feature_dim)
        self.nerv_formers = nn.ModuleList([
            NervFormerV2_unit(
        ...
        # 添加输入投影层，用于调整残差的维度
        self.input_proj = nn.Linear(num_channels * sequence_length, num_channels * sequence_length)
        self.layer_norm = nn.LayerNorm([num_channels, sequence_length])

    def forward(self, x, subject_indices=0):
        # x 的形状为 [batch_size, num_channels, sequence_length]
        for i, model in enumerate(self.nerv_formers):
            residual = x  # 保存输入作为残差
            out, x_reconstructed = model(x, subject_indices)
            # 添加残差连接并进行层归一化
            if x_reconstructed.shape == residual.shape:
                x = residual + x_reconstructed
            else:
                # 如果形状不匹配，使用线性层调整
                residual_flat = residual.view(residual.size(0), -1)
                residual_proj = self.input_proj(residual_flat).view_as(x_reconstructed)
                x = residual_proj + x_reconstructed
            x = self.layer_norm(x)
            # 如果不是最后一层，继续迭代
            if i != self.num_layers - 1:
                x = x  # x 已经更新
...
def train_model(eeg_model, img_model, dataloader, optimizer, device, text_features_all, img_features_all):
    ...
    for batch_idx, (eeg_data, labels, text, text_features, img, img_features) in enumerate(dataloader):
        ...
        # print("eeg_data:: ", eeg_data.shape) #torch.Size([1024, 63, 250])
        # eeg_data = eeg_data.unsqueeze(1)
        # print("eeg_data_afsq:: ", eeg_data.shape) #torch.Size([1024, 63, 250])

        text_features = text_features.to(device).float()
        # print("txt_data:: ", text_features.shape)# torch.Size([1024, 1024])

        img_features = img_features.to(device).float()
        # print("img_features:: ", img_features.shape) #  torch.Size([1024, 1024])
...
def main_train_loop(sub, current_time, eeg_model, img_model, train_dataloader, test_dataloader, optimizer, device, 
    ...
    for epoch in range(config['epochs']):
        ...
        # _, v200_acc, v200_top5_acc = evaluate_model(eeg_model, img_model, test_dataloader, device, text_features_test_all, img_features_test_all,  k=200)
...
def main():  
    config = {
        ...
        "logger": True,
        "insubject": True,
        "encoder_type": 'StackedNervFormerV2_4L_res',
    ...
    for sub in subjects:                    
        ...
        # eeg_model.apply(init_weights) # use Kaiming init
