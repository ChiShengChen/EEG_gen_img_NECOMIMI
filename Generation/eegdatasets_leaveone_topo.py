...
# import open_clip
# vlmodel, preprocess_train, feature_extractor = open_clip.create_model_and_transforms(
#     'hf-hub:laion/CLIP-ViT-H-14-laion2B-s32B-b79K', precision='fp32', device = device)
# tokenizer = open_clip.get_tokenizer('hf-hub:laion/CLIP-ViT-H-14-laion2B-s32B-b79K')
import json

# Load the configuration from the JSON file
config_path = "/home/aidan/EEG_Image_decode_old/Generation/data_config.json"
...
class EEGDataset():
    ...
    """
    def __init__(self, topo, data_path, exclude_subject=None, subjects=None, train=True, time_window=[0, 1.0], classes = None, pictures = None):
    ...
    def filter_eeg_by_topo_train(self, eeg_data, ch_names):
        if self.topo.lower() == 'all':  # Handle case-insensitivity
            return eeg_data
        else:
            electrodes = self.electrode_regions.get(self.topo.lower(), [])  # Ensure topo is in lowercase to match keys
            indices = [self.ch_names.index(elec) for elec in electrodes if elec in ch_names]
            if len(indices) > 0:
    ...
    def filter_eeg_by_topo(self, eeg_data, ch_names):
        if self.topo.lower() == 'all':  # Handle case-insensitivity
            return eeg_data
        else:
            electrodes = self.electrode_regions.get(self.topo.lower(), [])  # Ensure topo is in lowercase to match keys
            ...
            if self.train:
                ...
                if self.classes is not None and self.pictures is not None:
                    ...
                    for i in range(n_classes):
                        ...
                        preprocessed_eeg_data_class = self.filter_eeg_by_topo_train(preprocessed_eeg_data_class, self.ch_names)
                        data_list.append(preprocessed_eeg_data_class)
                        label_list.append(labels)
                ...
                if subject == self.exclude_subject or self.exclude_subject==None:
                    ...
                    for i in range(n_classes):
                        ...
                        preprocessed_eeg_data_class = self.filter_eeg_by_topo(preprocessed_eeg_data_class, self.ch_names)
                        # print("preprocessed_eeg_data_class", preprocessed_eeg_data_class.shape)
                        data_list.append(preprocessed_eeg_data_class)
...
if __name__ == "__main__":
    ...
    data_path = data_path
    train_dataset = EEGDataset("Frontal", data_path, subjects = ['sub-01'], train=True)    
    test_dataset = EEGDataset("Frontal", data_path, subjects = ['sub-01'], train=False)
